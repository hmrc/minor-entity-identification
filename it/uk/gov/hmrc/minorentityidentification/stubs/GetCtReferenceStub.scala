...
trait GetCtReferenceStub extends WiremockMethods {
  ...
  def stubGetCtReference(ctutr: String)(status: Int, body: JsValue = Json.obj()): StubMapping =
    when(method = GET, uri = s"/corporation-tax/identifiers/utr/$ctutr", headers = desHeaders)
